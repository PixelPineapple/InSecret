...
namespace team22.Object
{
    class UpElevator : GameObject
    {
        ...
        private int motionNum;          //光のためのモーションナンバー
        ...
        public UpElevator(Vector2 worldPosition, int floorWidth, int floorHeight, GameDevice gameDevice, GameObject target)
        ...
        public UpElevator(UpElevator other)
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            {
                Initialize();
                player.SetWorldPosition(new Vector2(GetRectangle().Center.X - player.GetRectangle().Width / 2, player.GetWorldPosition().Y - 32 * 10));
        ...
        public override void Draw(Renderer renderer)
        {
            ...
            if (isClose)
            {
                renderer.DrawTexture(playerName, new Vector2(drawingPosition.X + 15 + 20, drawingPosition.Y + 13 + 20), playerRect);
        ...
        private void MoveCamera()
        {
            ...
            if (camera.Position.Y <= GetWorldPosition().Y - 32 * 7)
            {
                isMovePlayer = true;
                isMoveCamera = false;
            }
            camera.SetPositionY(GetWorldPosition().Y - 32 * 7);
